...
import { getTrafficSignals, getWaterLevels, getMonitoringStatus, getGlobalThreatScore, getEmergencyIncidents } from '../services/infraAPI';
import { useAuth } from '../context/AuthContext';
import '../styles/ZoneMapNew.css';

const ZoneMapNew = () => {
  const { globalThreatScore, updateGlobalThreatScore, deviceTrustScore, user } = useAuth();
  ...
  return (
    <div className="zone-map-container">
      <div className="zone-map">
        <MapContainer center={[40.76, -73.94]} zoom={12} scrollWheelZoom={false}>
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            attribution='&copy; OpenStreetMap contributors'
          />
          <GeoJSON data={zoneGeoJSON} onEachFeature={onEachZone} style={setStyleToZone} />
          
          {signals.map((sig, i) => {
            const coords = [40.76 + (Math.random() - 0.5) * 0.1, -73.94 + (Math.random() - 0.5) * 0.1];
          ...
          {incidents.map((inc, i) => {
            const coords = [40.76 + (Math.random() - 0.5) * 0.15, -73.94 + (Math.random() - 0.5) * 0.15];
            return (
              <Marker key={`inc-${i}`} position={coords}>
      ...
      <div className="zone-details-panel">
        ...
        <div className="global-status">
          <h4>Global Status</h4>
          <p>Threat Score: {globalThreatScore.toFixed(0)}</p>
